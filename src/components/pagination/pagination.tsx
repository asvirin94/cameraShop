...
import {
  PAGES_COUNT_IN_PAGGINATION_SECTOR,
  PRODUCTS_ON_PAGE_COUNT,
} from '../../consts';
...
export default function Pagination() {
  ...
  const currentSector = Math.floor(
    currentPage / PAGES_COUNT_IN_PAGGINATION_SECTOR
  );

  const sortTypeUrl = sortType ? `&sortType=${sortType}` : '';
  const sortDirectionUrl = sortDirection
  ...
  const sectorsCount = Math.ceil(
    pagesCount / PAGES_COUNT_IN_PAGGINATION_SECTOR
  );
  const pagginationLength = Math.min(
    PAGES_COUNT_IN_PAGGINATION_SECTOR,
    pagesCount - currentSector * PAGES_COUNT_IN_PAGGINATION_SECTOR
  );
  const pagginationArray = Array.from({ length: pagginationLength });
  const nextButtonElement =
    currentSector === sectorsCount - 1 ? null : (
      <li
        className="pagination__item"
        onClick={() => {
          dispatch(
            setCurrentPage(
              (currentSector + 1) * PAGES_COUNT_IN_PAGGINATION_SECTOR
            )
          );
          window.scrollTo({
            top: 360,
  ...
  const prevButtonElement =
    currentSector === 0 ? null : (
      <li
        className="pagination__item"
        onClick={() => {
          dispatch(
            setCurrentPage(
              (currentSector - 1) * PAGES_COUNT_IN_PAGGINATION_SECTOR + 2
            )
          );
          window.scrollTo({
            top: 360,
  ...
  return products.length < 10 ? null : (
